...
# ===========================
cpdef __bkp__convolute(X, dists, node_indexes, value, leave_id, feature, threshold, verbose):
...
#cpdef convolute(X, dists, node_indexes, value, leave_id, feature, threshold, verbose):
    #golem = cGolem(X, dists, node_indexes, value, leave_id, feature, threshold, verbose)
#    golem._get_bboxes(X, dists, node_indexes, value, leave_id, feature, threshold, verbose)
#    golem._convolute()
#    return golem.np_y_robust, golem.np_y_robust_std, golem.np_bounds, golem.np_preds
...
@cython.boundscheck(False)
cpdef _get_bboxes(double [:, :] X, int [:, :] node_indexes, double [:] value, long [:] leave_id, long [:] feature,
                      double [:] threshold):
...
@cython.boundscheck(False)  # Deactivate bounds checking
@cython.wraparound(False)   # Deactivate negative indexing.
cpdef _convolute(double [:, :] X, BaseDist [:] dists, double [:] preds, double [:, :, :] bounds):
    start = time.time()
